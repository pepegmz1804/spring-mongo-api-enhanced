...
public interface RoleApiDoc {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Role created successfully",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "Created Role Example", value = """
                                        {
                                            "id": 15,
                                            "key": "admin_finance",
                                            "name": "Admin Finance"
            ...
            @ApiResponse(responseCode = "400",
                    ...
                    content = @Content),
            @ApiResponse(responseCode = "409", description = "Role key or name already exists",
                    content = @Content)})
    ResponseEntity<RoleResponseDTO> create(@Parameter(
            description = "DTO containing role key and name", required = true) RoleRequestDTO role);
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200",
            ...
            content = @Content(mediaType = "application/json", examples = @ExampleObject(
                    name = "Example Response", summary = "Sample paginated roles", value = """
                            {
                                "content": [
    ...
    ResponseEntity<Page<RoleResponseDTO>> getAll(
            ...
            @Parameter(description = "Number of roles per page", example = "10") int size,

            @Parameter(description = "Field by which to sort", example = "id") String sortBy,
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Role found successfully",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = RoleResponseDTO.class))),
            @ApiResponse(responseCode = "404", description = "Role not found", content = @Content)})
    ResponseEntity<RoleResponseDTO> getById(
            @Parameter(description = "ID of the role", required = true, example = "1") Long id);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Matching roles found",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(name = "Filtered Roles Example",
                                    ...
                                    value = """
                                            ...
                                            """))),
            @ApiResponse(responseCode = "404", description = "No roles matched the criteria",
                    content = @Content)})
    ResponseEntity<Page<RoleResponseDTO>> searchByName(
            @Parameter(description = "Substring to search in role names", required = true,
                    example = "admin") String name,

            @Parameter(description = "Page number (0-based index)", example = "0") int page,

            @Parameter(description = "Number of roles per page", example = "10") int size,

            @Parameter(description = "Field to sort by", example = "name") String sortBy,
