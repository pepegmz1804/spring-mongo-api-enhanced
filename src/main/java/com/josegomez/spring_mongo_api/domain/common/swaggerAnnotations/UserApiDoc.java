...
public interface UserApiDoc {
        ...
        @ApiResponses({@ApiResponse(responseCode = "200", description = "User created successfully",
        ...
        @ApiResponses({@ApiResponse(responseCode = "200", description = "User found",
                        ...
                        @ApiResponse(responseCode = "404", description = "User not found",
                                        content = @Content)})
        ResponseEntity<UserResponseDTO> getById(@Parameter(description = "User ID", example = "1",
                        required = true) Long id);

        // SEARCH BY NAME
        @Operation(summary = "Search users by name",
                        description = "Searches for users by name substring (case-insensitive). Supports pagination and sorting.")
        @ApiResponses({@ApiResponse(responseCode = "200", description = "Matching users found",
                        content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = UserResponseDTO.class))),
                        @ApiResponse(responseCode = "404",
                                        description = "No users matched the search",
                                        content = @Content)})
        ...
        @ApiResponses({@ApiResponse(responseCode = "204", description = "User deleted successfully",
                        ...
                        @ApiResponse(responseCode = "404", description = "User not found",
                                        content = @Content)})
        ResponseEntity<Void> delete(@Parameter(description = "ID of the user to delete",
                        example = "5", required = true) Long id);
}
